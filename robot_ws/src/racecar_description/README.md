...
## 完成过程
首先写了小车和传感器的xarco文件，最开始用的是差速转向，但是想到使用差速转向无法使用到之前控制学习的自行车模型和跟踪算法，于是最后选择了阿克曼转向。

接着，我阅读了车队给的赛道文件和视觉感知功能包（但好像加密了），编写了world文件,然后我就开始编写launch文件和桥接文件，编译启动后发现小车没有在gazebo中显示（解决在下面），解决了这个问题后，可以显示车体了，但是显示不了轮子，后来发现是因为桥接中缺少了关节状态。

但是还是无法显示相机和雷达的数据，于是让agent进行了检查，发现是gazebo和urdf的前缀不同导致的，使用frame_prefix为传感器和车子都加上了前缀，然后gazebo中的雷达和相机话题就可以正常发布了，也能够显示点云和照片了，但是点云扫到锥桶上只有一两个点，于是换成了3维的输出桥接，但是此时发现点云又出现了问题，经检查发现，PointCloud的桥接与frame_prefix的方式不兼容，遵循ai建议，对相机和点云进行了硬编码加前缀，最后所有的传感器都显示正常了，小车经测试也可以正常移动
## 快速开始
...
使用 Gazebo 内置的 `AckermannSteering` 插件：
...
### 锥桶模型的导入
最开始对锥桶模型的导入一脸懵，我询问了AI该如何解决相关问题，最后使用model://绝对路径的方法把锥桶直接写入了世界文件中。
